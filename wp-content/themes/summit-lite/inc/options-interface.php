...
function summit_optionsframework_tabs() {
	$counter = 0;
...
function summit_optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Uploader
		case "upload":
			$output .= summit_optionsframework_uploader( $value['id'], $val, null );
		...
		case 'background':
			...
			$output .= summit_optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
